...
Declared initial gravestones are `none` cells, which cannot hold a plant; give `ground` for an activation once one has been destroyed. Plank rows are declared; the deck edge and the shore column are observed on the board, which is why they are recorded as observed inputs. The descriptions are generated by an exporter, outside this repository, from the decoded level definitions, which are not redistributed; every bundled description regenerates identically from its recorded inputs. A level whose file is not at hand can be described by hand from the seed-selection screen, which shows the bans, and from the board.
